...
namespace Hood.SchemaTool
{
    internal static class Program
    {
        private static int Main(string[] args)
        {
            ...
            string connection =
                GetArg(args, "--connection", "-c")
                ?? Environment.GetEnvironmentVariable("HOOD_CONNECTION");
                ...
                case "upgrade":
                    if (string.IsNullOrWhiteSpace(connection))
                    {
                        Console.Error.WriteLine(
                            "error: a connection string is required (--connection or HOOD_CONNECTION)."
                        );
                        return 2;
                    }

                    DatabaseDeployResult result = new DatabaseDeployService().Deploy(
                        connection,
                        scripts
                    );
        ...
        private static bool IsHelp(string arg) => arg is "-h" or "--help" or "help" or "-?" or "/?";
        ...
        {
            Console.WriteLine(
                @"hood-schema — apply the Hood CMS database schema (idempotent, forward-only).
