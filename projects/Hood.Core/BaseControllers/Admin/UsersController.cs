...
namespace Hood.Admin.BaseControllers
{
    public abstract class UsersController : BaseController
    {
        ...
        public virtual async Task<IActionResult> Index(
            UserListModel<UserProfileView<IdentityRole>> model
        )
        {
        ...
        public virtual async Task<IActionResult> Edit(string id)
        {
            ...
            var roles = await _account.GetRolesAsync(
                new RoleListModel<IdentityRole> { PageIndex = 0, PageSize = int.MaxValue }
            );
        ...
        public virtual async Task<Response> Create(AdminCreateUserViewModel model)
        {
            try
            {
                ...
                if (model.NotifyUser)
                {
                    ...
                    {
                        // roll back!
                        ApplicationUser deleteUser = await _account.GetUserByEmailAsync(
                            model.Email
                        );
        ...
        public virtual async Task<Response> DeleteRole(string id)
        {
            try
            {
                ...
                {
                    throw new Exception(
                        $"The role Id {id} could not be found, therefore could not be deleted."
                    );
                }
            ...
            {
                return await ErrorResponseAsync<UsersController>(
                    $"Error deleting a role via the admin panel.",
                    ex
                );
            }
        }
        ...
        public virtual async Task<Response> SetRole(string id, string role, bool add)
        {
            try
            {
                ...
                var roles = new List<IdentityRole>() { IdentityRole };
                ...
                if (add)
                {
                    return await SuccessResponseAsync<UsersController>(
                        $"The user has been added the {role} role."
                    );
                }
                else
                {
                    return await SuccessResponseAsync<UsersController>(
                        $"The user has been removed from the {role} role."
                    );
                }
            }
            catch (Exception ex)
