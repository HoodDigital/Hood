...
namespace Hood.Extensions
{
    public static class HtmlHelperExtensions
    {
            ...
            where TModel : SpamPreventionModel
        {
            ...
            var output =
                $@"<div class='comments_or_notes hidden d-none'><label for='comments_or_notes_{hp_key}'>Comments:</label><input name='{SpamPreventionModel.HoneypotFieldName}' id='comments_or_notes_{hp_key}' type='text' tabindex='-1' autocomplete='off' /></div>";
            output +=
                $@"<input name='{SpamPreventionModel.TimestampFieldName}' id='ts_{hp_key}' value='{timestamp}' type='hidden' tabindex='-1' autocomplete='off' />";
            output +=
                $@"<input name='{SpamPreventionModel.HashFieldName}' id='hs_{hp_key}' value='{hash}' type='hidden' tabindex='-1' autocomplete='off' />";
            output +=
                $@"<input name='{SpamPreventionModel.SaltFieldName}' id='slt_{hp_key}' value='{salt}' type='hidden' tabindex='-1' autocomplete='off' />";
