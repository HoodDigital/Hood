...
namespace Hood.Services
{
    public class PropertyRepository : IPropertyRepository
    {
        ...
        {
            IQueryable<PropertyListingView> properties = _db.PropertyViews.Include(p => p.Metadata);

            if (model.LoadImages)
            {
                properties = properties.Include(p => p.Media).Include(p => p.FloorPlans);
            }
            ...
            model.AvailableTypes = await _db
                .Properties.Select(p => p.ListingType)
                .Distinct()
                .ToListAsync();
            model.AvailableStatuses = await _db
                .Properties.Select(p => p.LeaseStatus)
                .Distinct()
                .ToListAsync();
            model.AvailablePlanningTypes = await _db
                .Properties.Select(p => p.Planning)
                .Distinct()
                .ToListAsync();
        ...
        public async Task<PropertyListModel> GetFeaturedAsync()
        {
            ...
            else
            {
                properties = await GetPropertiesAsync(
                    new PropertyListModel() { Featured = true, PageSize = int.MaxValue }
                );
        ...
        public async Task<PropertyListModel> GetRecentAsync()
        {
            ...
            {
                properties = await GetPropertiesAsync(
                    new PropertyListModel() { PageSize = int.MaxValue, Order = "DateDesc" }
                );
                _cache.Add(cacheKey, properties, TimeSpan.FromMinutes(60));
                return properties;
            }
        ...
        {
            PropertyListing property = await _db
                .Properties.Include(p => p.Metadata)
                .Include(p => p.Media)
                .Include(p => p.FloorPlans)
                .Where(p => p.Id == id)
                .FirstOrDefaultAsync();
        ...
        public async Task<PropertyStatistics> GetStatisticsAsync()
        {
            ...
            int totalPublished = await _db
                ...
                .CountAsync();
            var data = await _db
                .Properties.Where(p => p.CreatedOn >= DateTime.Now.AddYears(-1))
                .Select(c => new
                {
