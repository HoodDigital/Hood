...
$.hood.Property = {
  Init: function Init() {
    ...
    if ($('#property-edit-form').doesExist()) $.hood.Property.Edit.Init();
  },
  Lists: {
    Property: {
    ...
    Media: {
      ...
      Reload: function Reload(complete) {
        if ($('#property-gallery-list').doesExist()) $.hood.Inline.Reload($('#property-gallery-list'), complete);
      }
    },
    Floorplans: {
      Loaded: function Loaded(data) {
      ...
      Reload: function Reload(complete) {
        if ($('#property-floorplans-list').doesExist()) $.hood.Inline.Reload($('#property-floorplans-list'), complete);
      }
    },
    Floorareas: {
      Loaded: function Loaded(data) {
      ...
      Reload: function Reload(complete) {
        if ($('#property-floors-list').doesExist()) $.hood.Inline.Reload($('#property-floors-list'), complete);
      }
    }
  },
  ...
  SetStatus: function SetStatus(e) {
    ...
    $.hood.Alerts.Confirm("The item will be immediately visible on the website.", "Are you sure?", publishPropertyCallback, 'warning');
  },
  ...
  CreateFloorArea: function CreateFloorArea() {
  ...
  RecalcFloor: function RecalcFloor(e) {
    if (this.id === "SquareMetres") $('#property-floorArea-create-form #SquareFeet').val(Number($(this).val()) * 10.7639);else {
  ...
  DeleteFloorArea: function DeleteFloorArea(e) {
    ...
    $.hood.Alerts.Confirm("The floor will be permanently removed.", "Are you sure?", deleteFloorAreaCallback, 'error', '<span class="text-danger"><i class="fa fa-exclamation-triangle"></i> <strong>This process CANNOT be undone!</strong></span>');
  },
  Edit: {
    Init: function Init() {
  ...
  Upload: {
    InitImageUploader: function InitImageUploader() {
      if (!$('#property-gallery-add').doesExist()) return;
      ...
      myDropzone.on("addedfile", function (file) {
        $('#property-gallery-total-progress .progress-bar').css({
          width: 0 + "%"
        });
    ...
    InitFloorplanUploader: function InitFloorplanUploader() {
      if (!$('#property-floorplans-add').doesExist()) return;
      ...
      myDropzone.on("addedfile", function (file) {
        $('#property-floorplans-total-progress .progress-bar').css({
          width: 0 + "%"
        });
