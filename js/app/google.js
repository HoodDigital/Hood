...
$.hood.Google = {
  ...
  Addresses: {
    ...
    InitAutocomplete: function InitAutocomplete() {
      this.autocomplete = new google.maps.places.Autocomplete(document.getElementById('address-autocomplete'), {
        types: ['geocode']
      });
    ...
    FillInAddress: function FillInAddress() {
      ...
      for (var component in $.hood.Google.Addresses.AddressForm) {
        if ($('#' + component).doesExist()) {
          $('#' + component).val('');
          var newVal = $.hood.Google.Addresses.AddressForm[component];
          var placeholders = $.hood.Google.GetPlaceholders(newVal);
          ...
          if (!newVal.contains('undefined')) $('#' + component).val(newVal);
        }
      }
    ...
    GetValueFromAddressComponents: function GetValueFromAddressComponents(key) {
  ...
  ClusteredMap: function ClusteredMap() {
    var mapElement = $('#clustered-map');
    if (!mapElement.length) return;
    var scrollWheel = false;
    if (mapElement.attr('data-scrollwheel')) scrollWheel = mapElement.data('scrollwheel');
    var zoom = false;
    if (mapElement.attr('data-zoom')) zoom = mapElement.data('zoom');
    ...
    var iconSize = new google.maps.Size(30, 41); // Create an array  of alphabetical characters used to label the markers.

    var labels = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
    var locations = mapElement.data('locations');
    var clickFunction = mapElement.data('click'); // Add some markers to the map.
    ...
    var markers = locations.map(function (location, i) {
      ...
      var info = '<div class="google-popup">' + '<img src="' + location.ImageUrl + '" class="map-image" />' + '<p>' + location.Address1 + ', ' + location.Postcode + '</p>' + '<p>' + location.Description + '</p>' + '</div>';
    ...
    var markerCluster = new MarkerClusterer(map, markers, {
      imagePath: 'https://cdn.jsdelivr.net/npm/hoodcms@4.0.0/images/maps/m',
      maxZoom: 15
    });
  },
  GetPlaceholders: function GetPlaceholders(str) {
    var regex = /\{(\w+)\}/g;
    var result = [];
...
function initGoogleMapsComplete() {
  ...
  $.hood.Google.Maps();
  $.hood.Google.ClusteredMap(); // try calling initMaps, this may have been added to pages.
